...
NumericMatrix SampleBetas(NumericMatrix param, NumericMatrix y, NumericMatrix xmat, 
                          double w,int nparam,int ncomm, NumericVector var1,double NBN) {
  ...
  for(int i=0;i<ncomm;i++){
    ...
    for (int j=0; j<nparam; j++){
      ...
      //define slice  
      rango1=DoublingBetas(LogMediaMiss,yslice,w,param(_,i),y(_,i),xmat,j,var1,NBN,LgammaNBN); //find range by doubling window
      
      //sample this particular parameter
      param(j,i)=SampleEachParamBetas(LogMediaMiss,rango1,yslice,param(_,i),y(_,i),xmat,j,var1,NBN); //sample within the defined range (rango1)
